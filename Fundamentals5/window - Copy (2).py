...
def update_taskbar(event):
    ...
    if canvas_height == 1080:
        ...
        taskbar_height = max(1, int( 37))  # Minimum value is 0.037

    print("taskbar_height: ", taskbar_height)
    resized_image = canvas.taskbar_image.resize((canvas.winfo_width(), 37), Image.Resampling.NEAREST)
    ...
    canvas.coords(canvas.taskbar_image_placed, 0, canvas.winfo_height() - 37)
