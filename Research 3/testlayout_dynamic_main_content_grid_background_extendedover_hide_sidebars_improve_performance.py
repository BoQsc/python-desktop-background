...
# A global variable to track the last resize event time
last_resize_time = 0
...
def create_layout_with_sidebar(canvas):
    ...
    # Check if the time elapsed since the last resize is greater than a threshold (e.g., 0.1 seconds)
    if current_time - last_resize_time < 0.1:
        return  # Skip redraw if the resize event is too frequent
...
# Bind the resizing event to update the layout with a debounce
