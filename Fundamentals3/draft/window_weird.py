...
class Taskbar:
    height = 40  # Set the height as a class attribute
    
    @property
    def width(self):
        # Return the current width of the canvas dynamically
    ...
    def update_image_position(self, window_event=None):
        if not hasattr(Taskbar, "image"):
            Taskbar.initialize_image()

        # Use the width property to get the current width of the canvas
        resized_image = Taskbar.image.resize((self.width, Taskbar.height), Image.Resampling.LANCZOS)
        print(self.width)  # Print the current width for verification
