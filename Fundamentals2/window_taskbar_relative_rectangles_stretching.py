...
def taskbar_image_resize(event):
    ...
    for i, rect in enumerate(colored_rectangles):
        rect_width = event.width // 5
        rect_x1 = i * rect_width
        rect_x2 = rect_x1 + rect_width
        rect_y1 = event.height - taskbar_height
        rect_y2 = event.height
        canvas.coords(rect, rect_x1, rect_y1, rect_x2, rect_y2)

# Create rectangles initially
colors = ["red", "green", "blue", "yellow", "orange"]
for i, color in enumerate(colors):
    rect = canvas.create_rectangle(0, 0, 0, 0, fill=color, outline="")
    colored_rectangles.append(rect)
