...
def taskbar_image_rectangles_resize(event):
    ...
    taskbar_area_width = event.width * 0.80  # 80% width for main area
...
# Add a "Start" button on the taskbar
start_button = tkinter.Button(window, text="Start", command=lambda: print("Start button clicked"))
start_button_window = canvas.create_window(10, window.winfo_height() - taskbar_height // 2, anchor="w", window=start_button)

def start_button_resize(event):
    # Update the position of the start button when the window resizes
    canvas.coords(start_button_window, 10, event.height - taskbar_height // 2)

on_window_event_callbacks.append(start_button_resize)
