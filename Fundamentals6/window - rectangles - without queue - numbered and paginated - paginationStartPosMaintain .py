...
def display_rectangles(num_visible, height, rect_width, padding):
    ...
    # Calculate the total number of fully visible rectangles
    visible_width = canvas.winfo_width() * 0.77  # taskbar area width
    max_visible = int((visible_width - padding) // (rect_width + padding))
    
    # Use current_page as the starting point
    start_index = current_page * max_visible
    
    # Display rectangles from start_index until either running out of space or rectangles
    x = padding  # Start from left padding
    i = start_index
    
    while i < len(rectangles) and x + rect_width < visible_width:
        x1 = x
        x2 = x + rect_width
        ...
        canvas.create_text((x1 + x2) / 2, (y1 + y2) / 2, text=str(i + 1), fill="white", tags="nums")
...
def on_key_press(event):
    ...
    visible_width = last_event.width * 0.77
    padding = 6
    rect_width = 50
    max_visible = int((visible_width - padding) // (rect_width + padding))
    max_pages = (len(rectangles) - 1) // max_visible + 1
